...
void helia_eqv_win ( uint16_t opacity, GtkWindow *win_base, GstElement *element )
{
	...
	GtkButton *button_all_clear = helia_create_button ( NULL, "helia-clear", "🗑", ICON_SIZE );
	...
	uint8_t c = 0; for ( c = 0; c < NUMS; c++ )
	{
		...
		GtkButton *button = helia_create_button ( hm_box, "helia-clear", "🗑", ICON_SIZE );
	...
	GtkButton *button = helia_create_button ( h_box, "helia-exit", "🞬", ICON_SIZE );
